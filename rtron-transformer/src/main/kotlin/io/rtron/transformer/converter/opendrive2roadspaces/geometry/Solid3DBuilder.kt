...
class Solid3DBuilder(
    ...
    fun buildCuboids(roadObject: RoadObjectsObject, curveAffine: Affine3D): List<Cuboid3D> {
        ...
        if (roadObject.repeat.isNotEmpty() && roadObject.repeat.first().isRepeatedCuboid()) // TODO fix repeat list handling
    ...
    fun buildCylinders(roadObject: RoadObjectsObject, curveAffine: Affine3D): List<Cylinder3D> {
        ...
        if (roadObject.isCylinder()) {
            val objectAffine = Affine3D.of(roadObject.referenceLinePointRelativePose)
            val affineSequence = AffineSequence3D.of(curveAffine, objectAffine)
            cylinderList += Cylinder3D(roadObject.radius.getOrElse { Double.NaN }, roadObject.height.getOrElse { Double.NaN }, configuration.numberTolerance, affineSequence) // TODO fix optional
        }

        if (roadObject.repeat.isNotEmpty() && roadObject.repeat.first().isRepeatCylinder()) // TODO fix repeat list handling
    ...
    private fun buildPolyhedronByRoadCorners(
        ...
        return Polyhedron3DFactory.buildFromVerticalOutlineElements(verticalOutlineElements, configuration.numberTolerance)
    }
    ...
    private fun buildVerticalOutlineElement(
        ...
        val verticalOutlineElement = Polyhedron3DFactory.VerticalOutlineElement(basePoint, headPoint, tolerance = configuration.numberTolerance)
    ...
    fun buildParametricSweeps(roadObject: RoadObjectsObject, roadReferenceLine: Curve3D): List<ParametricSweep3D> {
        ...
        if (!roadObject.repeat.first().isParametricSweep()) return emptyList() // TODO fix repeat list handling

        // curve over which the object is moved
        val objectReferenceCurve2D =
            _curve2DBuilder.buildLateralTranslatedCurve(roadObject.repeat.first(), roadReferenceLine) // TODO fix repeat list handling
                .toResult()
                .handleFailure { reportLogger.log(it.toEither(), roadObject.id); return emptyList() }
        val objectReferenceHeight =
            _functionBuilder.buildStackedHeightFunctionFromRepeat(roadObject.repeat.first(), roadReferenceLine) // TODO fix repeat list handling

        // dimensions of the sweep
        val heightFunction = roadObject.repeat.first().getObjectHeightFunction() // TODO fix repeat list handling
        val widthFunction = roadObject.repeat.first().getObjectWidthFunction() // TODO fix repeat list handling
