...
class LaneBuilder(
     ...
     */
    fun buildLane(id: LaneIdentifier, curvePositionDomain: Range<Double>, srcLane: RoadLanesLaneSectionLRLane,
                  baseAttributes: AttributeList): Lane {
        ...
        return Lane(id, width, laneHeightOffsets.inner, laneHeightOffsets.outer, srcLane.level, roadMarkings,
     ...
     * @param baseAttributes attributes attached to the transformed [Lane] object
     */
    fun buildCenterLane(id: LaneSectionIdentifier, curvePositionDomain: Range<Double>,
                        srcLanes: List<RoadLanesLaneSectionCenterLane>, baseAttributes: AttributeList): CenterLane {
        ...
        if (srcLanes.size > 1)
            _reportLogger.info("Lane section contains multiple center lanes, " +
                    "but should contain only one.", id.toString())
    ...
    private fun buildLaneHeightOffset(id: LaneIdentifier, srcLaneHeights: List<RoadLanesLaneSectionLRLaneHeight>):
        ...
        else ConcatenatedFunction.ofLinearFunctions(heightEntriesAdjusted.map { it.sOffset },
     ...
     */
    private fun buildRoadMarkings(id: LaneIdentifier, curvePositionDomain: Range<Double>,
                                  srcRoadMark: List<RoadLanesLaneSectionLCRLaneRoadMark>): List<RoadMarking> {
        ...
        if (adjustedSrcRoadMark.size < srcRoadMark.size)
            _reportLogger.warn("Road mark entries have been removed, as the sOffset is not located within " +
                    "the local curve position domain ($curvePositionDomain) of the lane section.", id.toString())
