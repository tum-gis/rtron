        ...
        modifiedOpendriveModel = everyLaneSection.modify(modifiedOpendriveModel) { currentLaneSection ->
            ...
            currentLaneSection.left.tap { currentLaneSectionLeft ->
                if (!currentLaneSectionLeft.lane.map { it.id }.isSortedDescending()) {
                    ...
                    messageList += DefaultMessage.of("", "Lane numbering shall start with 1 next to the center lane, descend in negative t-direction and ascend in positive t-direction.", currentLaneSection.additionalId, Severity.FATAL_ERROR, wasFixed = true)
                }
            }
