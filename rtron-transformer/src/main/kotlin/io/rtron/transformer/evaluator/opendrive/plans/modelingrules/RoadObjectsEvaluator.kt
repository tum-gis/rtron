        ...
        healedOpendriveModel = everyRoadObject.modify(healedOpendriveModel) { currentRoadObject ->
            ...
            currentRoadObject.outlines.tap { currentOutlinesElement ->
                ...
                if (outlineElementsWithoutId.isNotEmpty()) {
                    ...
                    messageList += OpendriveException.MissingValue("id").toMessage(currentRoadObject.additionalId, isFatal = false, wasHealed = true)
            ...
            currentRoadObject.outlines.tap { currentRoadObjectOutline ->
                if (currentRoadObjectOutline.outline.any { it.isPolyhedron() && !it.isPolyhedronUniquelyDefined() }) {
                    messageList += Message.of("An <outline> element shall be followed by one or more <cornerRoad> elements or by one or more <cornerLocal> element. Since both are defined, the <cornerLocal> elements are removed.", currentRoadObject.additionalId, isFatal = true, wasHealed = true)
            ...
            if (currentRoadObject.height.isEmpty() && currentRoadObject.outlines.exists { it.containsPolyhedrons() }) {
                messageList += Message.of("Road object contains a polyhedron with non-zero height, but the height of the road object element is ${currentRoadObject.height}.", currentRoadObject.additionalId, isFatal = false, wasHealed = false)
            }
            ...
            if (repeatElementsFiltered.size < currentRoadObject.repeat.size) {
                ...
                messageList += Message.of("A repeat element should have a length higher than zero and threshold.", currentRoadObject.additionalId, isFatal = true, wasHealed = true)
