...
class RoadLanesEvaluator(val configuration: OpendriveEvaluatorConfiguration) {
    ...
    // Methods
    fun evaluateFatalViolations(roadIdentifier: RoadIdentifier, roadLanes: RoadLanes, roadLength: Double): Report {
        val report = Report()

        if (roadLanes.getLaneSectionLengths(roadLength).any { it <= configuration.numberTolerance })
            report += Message.of("The length of lane sections shall be greater than zero.", roadIdentifier.toStringMap(), isFatal = true, wasHealed = false)
    ...
    fun evaluateNonFatalViolations(roadIdentifier: RoadIdentifier, roadLanes: RoadLanes, roadLength: Double): ContextReport<RoadLanes> {
