    ...
    fun evaluateNonFatalViolations(opendriveModel: OpendriveModel): ContextMessageList<OpendriveModel> {
        ...
        healedOpendriveModel = everyRoadObject.modify(healedOpendriveModel) { currentRoadObject ->
            ...
            if (!currentRoadObject.s.isFinite() || currentRoadObject.s < 0.0) {
                messageList += OpendriveException.UnexpectedValue("s", currentRoadObject.s.toString(), "Value shall be finite and greater equals zero.").toMessage(currentRoadObject.additionalId, isFatal = true, wasHealed = false)
                currentRoadObject.s = 0.0
            }
            if (!currentRoadObject.t.isFinite()) {
                messageList += OpendriveException.UnexpectedValue("t", currentRoadObject.t.toString(), "Value shall be finite.").toMessage(currentRoadObject.additionalId, isFatal = true, wasHealed = false)
                currentRoadObject.t = 0.0
            }
            if (!currentRoadObject.zOffset.isFinite()) {
                messageList += OpendriveException.UnexpectedValue("zOffset", currentRoadObject.zOffset.toString(), "Value shall be finite.").toMessage(currentRoadObject.additionalId, isFatal = true, wasHealed = false)
                currentRoadObject.zOffset = 0.0
            }

            if (currentRoadObject.hdg.exists { !it.isFinite() }) {
                messageList += OpendriveException.UnexpectedValue("hdg", currentRoadObject.hdg.toString(), "Value shall be finite.").toMessage(currentRoadObject.additionalId, isFatal = true, wasHealed = false)
                currentRoadObject.hdg = None
            }

            if (currentRoadObject.roll.exists { !it.isFinite() }) {
                messageList += OpendriveException.UnexpectedValue("roll", currentRoadObject.roll.toString(), "Value shall be finite.").toMessage(currentRoadObject.additionalId, isFatal = true, wasHealed = false)
                currentRoadObject.roll = None
            }

            if (currentRoadObject.pitch.exists { !it.isFinite() }) {
                messageList += OpendriveException.UnexpectedValue("pitch", currentRoadObject.pitch.toString(), "Value shall be finite.").toMessage(currentRoadObject.additionalId, isFatal = true, wasHealed = false)
                currentRoadObject.roll = None
            }

            if (currentRoadObject.height.exists { !it.isFinite() || it < 0.0 }) {
                messageList += OpendriveException.UnexpectedValue("height", currentRoadObject.height.toString(), "Value shall be finite and greater equals zero.").toMessage(currentRoadObject.additionalId, isFatal = true, wasHealed = false)
            ...
            if (currentRoadObject.radius.exists { !it.isFinite() || it <= configuration.numberTolerance }) {
                messageList += OpendriveException.UnexpectedValue("radius", currentRoadObject.radius.toString(), "Value shall be finite and greater zero.").toMessage(currentRoadObject.additionalId, isFatal = true, wasHealed = false)
                currentRoadObject.radius = None
            }

            if (currentRoadObject.length.exists { !it.isFinite() || it <= configuration.numberTolerance }) {
                messageList += OpendriveException.UnexpectedValue("length", currentRoadObject.length.toString(), "Value shall be finite and greater zero.").toMessage(currentRoadObject.additionalId, isFatal = true, wasHealed = false)
                currentRoadObject.length = None
            }

            if (currentRoadObject.width.exists { !it.isFinite() || it <= configuration.numberTolerance }) {
                messageList += OpendriveException.UnexpectedValue("width", currentRoadObject.width.toString(), "Value shall be finite and greater zero.").toMessage(currentRoadObject.additionalId, isFatal = true, wasHealed = false)
                currentRoadObject.width = None
            }

            if (currentRoadObject.outlines.exists { it.outline.isEmpty() }) {
                messageList += OpendriveException.EmptyValueForOptionalAttribute("outlines").toMessage(currentRoadObject.additionalId, isFatal = true, wasHealed = false)
        ...
        healedOpendriveModel = everyRoadObjectOutlineElement.modify(healedOpendriveModel) { currentOutlineElement ->
            ...
            currentOutlineElement.cornerRoad.forEach { currentCornerRoad ->
                if (!currentCornerRoad.height.isFinite() || currentCornerRoad.height < 0.0) {
                    messageList += OpendriveException.UnexpectedValue("height", currentCornerRoad.height.toString(), "Value shall be finite and greater equals zero.").toMessage(currentOutlineElement.additionalId, isFatal = true, wasHealed = false)
            ...
            currentOutlineElement.cornerLocal.forEach { currentCornerLocal ->
                if (!currentCornerLocal.height.isFinite() || currentCornerLocal.height < 0.0) {
                    messageList += OpendriveException.UnexpectedValue("height", currentCornerLocal.height.toString(), "Value shall be finite and greater equals zero.").toMessage(currentOutlineElement.additionalId, isFatal = true, wasHealed = false)
        ...
        healedOpendriveModel = everyRoadObjectRepeatElement.modify(healedOpendriveModel) { currentRepeatElement ->
            ...
            if (!currentRepeatElement.distance.isFinite() || currentRepeatElement.distance < 0.0) {
                messageList += OpendriveException.UnexpectedValue("distance", currentRepeatElement.distance.toString(), "Value shall be finite and greater equals zero.").toMessage(currentRepeatElement.additionalId, isFatal = true, wasHealed = false)
                currentRepeatElement.distance = 0.0
            }
            if (!currentRepeatElement.heightEnd.isFinite() || currentRepeatElement.heightEnd < 0.0) {
                messageList += OpendriveException.UnexpectedValue("heightEnd", currentRepeatElement.heightEnd.toString(), "Value shall be finite and greater equals zero.").toMessage(currentRepeatElement.additionalId, isFatal = true, wasHealed = false)
                currentRepeatElement.heightEnd = 0.0
            }
            if (!currentRepeatElement.heightStart.isFinite() || currentRepeatElement.heightStart < 0.0) {
                messageList += OpendriveException.UnexpectedValue("heightStart", currentRepeatElement.heightStart.toString(), "Value shall be finite and greater equals zero.").toMessage(currentRepeatElement.additionalId, isFatal = true, wasHealed = false)
                currentRepeatElement.heightStart = 0.0
            }
            if (!currentRepeatElement.length.isFinite() || currentRepeatElement.length < 0.0) {
                messageList += OpendriveException.UnexpectedValue("length", currentRepeatElement.length.toString(), "Value shall be finite and greater equals zero.").toMessage(currentRepeatElement.additionalId, isFatal = true, wasHealed = false)
                currentRepeatElement.length = 0.0
            }
            if (currentRepeatElement.lengthEnd.exists { !it.isFinite() || it <= configuration.numberTolerance }) {
                messageList += OpendriveException.UnexpectedValue("lengthEnd", currentRepeatElement.lengthEnd.toString(), "Value shall be finite and greater zero.").toMessage(currentRepeatElement.additionalId, isFatal = true, wasHealed = false)
            ...
            if (currentRepeatElement.lengthStart.exists { !it.isFinite() || it <= configuration.numberTolerance }) {
                messageList += OpendriveException.UnexpectedValue("lengthStart", currentRepeatElement.lengthStart.toString(), "Value shall be finite and greater zero.").toMessage(currentRepeatElement.additionalId, isFatal = true, wasHealed = false)
            ...
            if (currentRepeatElement.radiusEnd.exists { !it.isFinite() || it <= configuration.numberTolerance }) {
                messageList += OpendriveException.UnexpectedValue("radiusEnd", currentRepeatElement.radiusEnd.toString(), "Value shall be finite and greater zero.").toMessage(currentRepeatElement.additionalId, isFatal = true, wasHealed = false)
            ...
            if (currentRepeatElement.radiusStart.exists { !it.isFinite() || it <= configuration.numberTolerance }) {
                messageList += OpendriveException.UnexpectedValue("radiusStart", currentRepeatElement.radiusStart.toString(), "Value shall be finite and greater zero.").toMessage(currentRepeatElement.additionalId, isFatal = true, wasHealed = false)
                currentRepeatElement.radiusStart = None
            }
            if (!currentRepeatElement.tEnd.isFinite()) {
                messageList += OpendriveException.UnexpectedValue("tEnd", currentRepeatElement.tEnd.toString(), "Value shall be finite.").toMessage(currentRepeatElement.additionalId, isFatal = true, wasHealed = false)
            ...
            if (currentRepeatElement.widthEnd.exists { !it.isFinite() || it <= configuration.numberTolerance }) {
                messageList += OpendriveException.UnexpectedValue("widthEnd", currentRepeatElement.widthEnd.toString(), "Value shall be finite and greater zero.").toMessage(currentRepeatElement.additionalId, isFatal = true, wasHealed = false)
                currentRepeatElement.widthEnd = None
            }
            if (currentRepeatElement.widthStart.exists { !it.isFinite() || it <= configuration.numberTolerance }) {
                messageList += OpendriveException.UnexpectedValue("widthStart", currentRepeatElement.widthStart.toString(), "Value shall be finite and greater zero.").toMessage(currentRepeatElement.additionalId, isFatal = true, wasHealed = false)
                currentRepeatElement.widthStart = None
            }
            if (!currentRepeatElement.zOffsetEnd.isFinite()) {
                messageList += OpendriveException.UnexpectedValue("zOffsetEnd", currentRepeatElement.zOffsetEnd.toString(), "Value shall be finite.").toMessage(currentRepeatElement.additionalId, isFatal = true, wasHealed = false)
