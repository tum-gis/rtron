 ...
 * @param roadMarks list of road markings, which are placed on the outer lane boundary
...
data class Lane(
    ...
    fun getLaneChange(): Option<LaneChange> {
        if (roadMarks.isEmpty()) {
            return None
        }

        val laneChangeSet = roadMarks.map { it.laneChange }.toSet()
 ...
 * @param roadMarks list of road markings, which are placed in the middle of the center lane
 * @param attributes information attributes to the lane
 */
data class CenterLane(
    val id: LaneIdentifier,
    val level: Boolean = false,
    val roadMarks: List<RoadMark> = emptyList(),
