...
class SubcommandOpendriveToCitygml2 : CliktCommand(name = "opendrive-to-citygml2", help = "Transform OpenDRIVE to CityGML 2.0.", printHelpOnEmptyArgs = true) {
    ...
    override fun run() {
        ...
        processAllFiles(inInputDirectory = inputPath.toString(), withExtension = OpendriveReader.supportedFileExtensions.head, toOutputDirectory = outputPath.toString()) {
            ...
            val roadspacesModel = transformOpendrive2Roadspaces(opendriveModel) {
                this@SubcommandOpendriveToCitygml2.tolerance?.let { numberTolerance = it }
                this@SubcommandOpendriveToCitygml2.crsEpsg?.let { crsEpsg = it }
                offset?.let {
                    offsetX = it.first
                    offsetY = it.second
                    offsetZ = it.third
            ...
            val citygmlModel = transformRoadspaces2Citygml(roadspacesModel) {
                ...
                this@SubcommandOpendriveToCitygml2.discretizationStepSize?.let { discretizationStepSize = it }
                this@SubcommandOpendriveToCitygml2.sweepDiscretizationStepSize?.let { sweepDiscretizationStepSize = it }
                this@SubcommandOpendriveToCitygml2.circleSlices?.let { circleSlices = it }
                transformAdditionalRoadLines = this@SubcommandOpendriveToCitygml2.transformAdditionalRoadLines
