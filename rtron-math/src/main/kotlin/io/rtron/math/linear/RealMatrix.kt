...
class RealMatrix(
    ...
    private val _matrix: Array2DRowRealMatrix = Array2DRowRealMatrix(rows)

    /** entry values of matrix whereby each [DoubleArray] represents a row */
    val entries: Array<DoubleArray> by lazy { _matrix.data }
    ...
    /** number of rows */
    val rowDimension = _matrix.rowDimension

    /** number of columns */
    val columnDimension = _matrix.columnDimension
    ...
    fun getRow(rowIndex: Int): DoubleArray = _matrix.getRow(rowIndex)!!

    /** Returns the row of [rowIndex] as a real matrix. */
    fun getRowMatrix(rowIndex: Int): RealMatrix = RealMatrix(_matrix.getRowMatrix(rowIndex))

    /** Returns the row of [rowIndex] as real vector. */
    fun getRowVector(rowIndex: Int): RealVector = _matrix.getRowVector(rowIndex).toRealVector()

    /** Returns the column of [columnIndex] as double array. */
    fun getColumn(columnIndex: Int): DoubleArray = _matrix.getColumn(columnIndex)!!

    /** Returns the column of [columnIndex] as a real matrix. */
    fun getColumnMatrix(columnIndex: Int): RealMatrix = RealMatrix(_matrix.getColumnMatrix(columnIndex))
    ...
    fun toDoubleList(): List<Double> = _matrix.data.flatMap { it.asIterable() }.toList()

    /** Conversion to adapted Real Matrix class from Apache Commons Math. */
    fun toRealMatrixCM(): CMRealMatrix = _matrix
