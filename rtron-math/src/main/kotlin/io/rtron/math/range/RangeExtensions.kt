 ...
 */
fun <T : Comparable<*>> Set<Range<T>>.intersectingRange(): Range<T> = reduce { acc, element -> acc.intersection(element) }

/**
 * Union operation of a set of [Range] to a [RangeSet].
 */
fun <T : Comparable<*>> Set<Range<T>>.unionRanges(): RangeSet<T> = map { RangeSet.of(it) }.toSet().unionRangeSets()
