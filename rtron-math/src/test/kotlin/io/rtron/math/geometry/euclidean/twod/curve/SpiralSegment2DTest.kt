            ...
            test("second spiral of the ASAM example dataset Ex_Line-Spiral-Arc") {
                ...
                val curvatureFunction =
                    LinearFunction.ofSpiralCurvature(
                        1.3333327910466574e-02,
                        6.6666666666666671e-03,
            ...
            test("test point and rotation calculation against csv sample dataset") {
                ...
                val records: Iterable<CSVRecord> =
                    CSVFormat.Builder
                        ...
                        .setDelimiter(
                            ",",
                        ).setHeader("s0", "x0", "y0", "hdg0", "curv0", "curv1", "length", "s1", "x1", "y1", "hdg1")
                        .setSkipHeaderRecord(
                            true,
                        ).get()
                        .parse(fileReader)
                for (record in records) {
                    val pose =
                        Pose2D(
                            Vector2D(record.get("x0").toDouble(), record.get("y0").toDouble()),
                            Rotation2D(record.get("hdg0").toDouble()),
                        )
                    val affine = Affine2D.of(pose)
                    val curvatureFunction =
                        LinearFunction.ofSpiralCurvature(
