...
class CubicCurve2DTest :
    FunSpec({
        context("TestPoseCalculation") {
            ...
            /**
             * Cubic curve geometry from the poly3 example dataset of
            ...
            test("cubic curve geometry from OpenDRIVE poly3 example dataset with gap to the next curve element") {
                ...
                val pose =
                    Pose2D(
                        Vector2D(-2.6331198952545350e+01, -7.4309373646250769e+00),
                        Rotation2D(6.7579136200528211e-01),
                    )
                val affine = Affine2D.of(pose)
                val affineSequence = AffineSequence2D.of(affine)
                ...
                // not coincident with next curve element
                actualPose.point.x.shouldNotBe(-5.0558344684384622e+00 plusOrMinus DBL_EPSILON)
                // not coincident with next curve element
                actualPose.point.y.shouldNotBe(9.6260358855640789e+00 plusOrMinus DBL_EPSILON)
                // not coincident with next curve element
                actualPose.rotation.angle.shouldNotBe(3.8412114603351055e-01 plusOrMinus DBL_EPSILON)
            }
        }
    })
